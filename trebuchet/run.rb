...
sum_of_first_and_last_numbers = split_input.reduce(0) do |sum, string|
  input_stripped_of_character = string.gsub(/\D/, '')

  # if there is only one number first and last number would be the same
  first_number = input_stripped_of_character[0]
  last_number = input_stripped_of_character[-1]

  sum + (first_number + last_number).to_i
end

puts sum_of_first_and_last_numbers
